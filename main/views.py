...
def AddBalance(request,pk):
    ...
    customeObjs = Customer.objects.get(id=pk)

    context = {'cobj':customeObjs}
...
def UpdateBalance(request):
    ...
    if request.method == 'POST':
        ...
        balance = Customer.objects.get(id=cid).amount
        ...
        if user is not None:
         ...
         if customer.status==True:
            if customer.amount >= amount:
                ...
                customer.save()
                print(customer.id)
                Transaction.objects.create(customer=customer, amount=amount)
            ...
            messages.error(request, "You can't buy units, Contact your admin")
        else:
            messages.error(request, 'Incorect password')
    return redirect('../')

from django.http import JsonResponse
def postData(request,meterNO):
    print(meterNO)
    data =  Customer.objects.filter(meter_number=meterNO).exists()
    if data:
     datas = Customer.objects.get(meter_number=meterNO)

     return JsonResponse({'unit':datas.units})
